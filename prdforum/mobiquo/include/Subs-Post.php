...
{
	global $scripturl, $txt, $user_info, $language,$id_pm;
...
function createPost(&$msgOptions, &$topicOptions, &$posterOptions)
{
	...
	// Fix the attachments.
	if (!empty($msgOptions['attachments']) && is_array($msgOptions['attachments']))
    {
        $isValidattachArray = true;
        foreach($msgOptions['attachments'] as $attach)
        {
            if(empty($attach))
            {
                $isValidattachArray  = false;
            }
        }
        if($isValidattachArray)
        {
		    $smcFunc['db_query']('', '
