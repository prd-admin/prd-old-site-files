...
function ModifyFeatureSettings()
{
	...
	$context['admin_tabs']['tabs']['custom'] = array(
		'title' => $txt['custom_tab_mods_cat'],
		'href' => $scripturl . '?action=featuresettings;sa=custom;sesc=' . $context['session_id'],
		);
...
function ModifyCustomTabSettings($return_config = false)
{
	global $txt, $scripturl, $context, $settings, $sc;

	$config_vars = array(

		array('check', 'custom_tab_enable_guest_access'),
		array('check', 'custom_tab_enable_member_access'),
		array('text', 'custom_tab_label', '20'),
		array('text', 'custom_tab_execute_url', '60'),
	);
	...
	if (isset($_GET['save']))
	{
		...
		redirectexit('action=featuresettings;sa=custom');
	}

	$context['post_url'] = $scripturl . '?action=featuresettings2;save;sa=custom';
	$context['settings_title'] = $txt['custom_tab_title'];
