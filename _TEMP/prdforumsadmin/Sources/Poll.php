...
$poll = !empty($_REQUEST['poll']) ? (int)$_REQUEST['poll'] : 0;
	...
	$request = db_query("
		SELECT t.ID_MEMBER_STARTED, p.ID_POLL, p.votingLocked
		FROM {$db_prefix}topics AS t
	...
	else
	{
		...
		db_query("
			INSERT INTO {$db_prefix}polls
				(ID_TOPIC, question, hideResults, maxVotes, expireTime, ID_MEMBER, posterName, changeVote)
			VALUES ($topic, SUBSTRING('$_POST[question]', 1, 255), $_POST[poll_hide], $_POST[poll_max_votes], $_POST[poll_expire], $ID_MEMBER, SUBSTRING('$user_info[username]', 1, 255), $_POST[poll_change_vote])", __FILE__, __LINE__);
