	...
	if (empty($_REQUEST['sa']) || $_REQUEST['sa'] == 'options')
	{
		if (count($polls) > 1)
		{
			$request = db_query("
				...
				FROM ({$db_prefix}polls AS p LEFT JOIN {$db_prefix}topics AS t ON p.ID_TOPIC = t.ID_TOPIC LEFT JOIN {$db_prefix}messages AS m ON t.ID_FIRST_MSG = m.ID_MSG)
				WHERE p.ID_POLL IN (" . implode(', ', $polls) . ")
					" . /* REMEMBER TO RE-ADD CODE HERE 763248 */"
