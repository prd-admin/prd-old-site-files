...
function Display()
{
	...
	if ($context['is_poll'])
	{
		...
		$request = db_query("
			...
			WHERE p.ID_POLL = $topicinfo[ID_POLL] OR p.ID_TOPIC = $topic
			GROUP BY p.ID_POLL
			ORDER BY p.question", __FILE__, __LINE__);
		$pollinfo = array();
		while($row = mysql_fetch_assoc($request)) $pollinfo[] = $row;
		mysql_free_result($request);
		...
		$context['poll']['allow_vote'] = !$context['poll']['is_expired'] && !$user_info['is_guest'] && empty($poll['votingLocked']) && allowedTo('poll_vote') && !$context['poll']['has_voted'];
		...
		$context['poll']['allow_poll_view'] = allowedTo('moderate_board') || $poll['hideResults'] == 0 || ($poll['hideResults'] == 1 && $context['poll']['has_voted']) || $context['poll']['is_expired'];
		$context['poll']['show_results'] = $context['poll']['allow_poll_view'] && isset($_REQUEST['viewResults']) && $poll['ID_POLL'] == $_REQUEST['poll'];
		...
		$context['poll']['allow_change_vote'] = !$context['poll']['is_expired'] && !$user_info['is_guest'] && empty($poll['votingLocked']) && allowedTo('poll_vote') && $context['poll']['has_voted'] && $context['poll']['change_vote'];
		...
		foreach ($pollOptions as $i => $option)
		{
			...
			$context['poll']['options'][$i] = array(
				...
				'vote_button' => '<input type="' . ($poll['maxVotes'] > 1 ? 'checkbox' : 'radio') . '" name="options[]" id="options-' . $i . '" value="' . $i . '" class="check" />'
			);
		}
