...
function loadMemberData($users, $is_name = false, $set = 'normal')
{
	...
	if ($set == 'normal')
	{
		$select_columns = "
			...
			mem.birthdate, mem.memberIP, mem.memberIP2, mem.ICQ, mem.AIM, mem.YIM, mem.MSN, mem.posts, mem.lastLogin, mem.gmail,
	...
	elseif ($set == 'profile')
	{
		$select_columns = "
			...
			mem.signature, mem.personalText, mem.location, mem.gender, mem.avatar, mem.ID_MEMBER, mem.memberName,
			mem.realName, mem.emailAddress, mem.hideEmail, mem.dateRegistered, mem.websiteTitle, mem.websiteUrl,
			mem.birthdate, mem.ICQ, mem.AIM, mem.YIM, mem.MSN, mem.posts, mem.lastLogin, mem.karmaGood, mem.gmail,
...
function loadMemberContext($user)
{
	...
	$memberContext[$user] = array(
		...
		) : array('name' => '', 'href' => '', 'link' => '', 'link_text' => ''),'gmail' => $profile['gmail'] !='' && (empty($modSettings['guest_hideContacts']) || !$user_info['is_guest']) ? array(
			'name' => &$profile['gmail'],
			'href' => 'mailto:'.$profile['gmail'].'',
			'link' => '<a href="mailto:' . $profile['gmail'] . '" target="_blank"><img src="' . $settings['images_url'] . '/gmail.gif" alt="*" border="0" /></a>',
