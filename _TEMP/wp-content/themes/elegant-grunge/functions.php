...
function elegant_grunge_filter( $content , $arg2=null, $arg3=null ) {
	...
	// 'A' tags which we want to include inside the frame
	$aStart = '(?:<\s*a[^>]+>\s*)?';
	$aEnd = '(?:\s*</a\s*>)?';
	...
	$regex = "$aStart$startTagWithFrameClass(?:$endSingleTag|>$nestedContent$endOriginalTag)$aEnd";
	...
	if ( $frame_all_images ) {
		...
		$regex = "$aStart\s*<\s*(img)${notFrameClass}${skippedClasses}[^>]+>\s*$aEnd";
		$regexes[] = "@".$regex."@is";
	}
...
function elegant_grunge_replace($matches) {
	...
	return '<span class="frame-outer '.$small.' '.$classes.'"'.($styles ? ' style="'.$styles.'"' : '' ).'><span><span><span>' .
...
function image_setup($post) {
    ...
    if ( !preg_match_all("@(?P<tag>[a-zA-Z]+)(?:=([\"'])(?P<value>.+?)\\2)@", $matches[2], $tag) ) {
        return;
    } 

    $image = array_combine($tag["tag"], $tag["value"]);

    if ( $matches[1] && preg_match_all("@(?P<tag>[a-zA-Z]+)(?:=([\"'])(?P<value>.+?)\\2)@", $matches[1], $tag) ) {
        $link = array_combine($tag["tag"], $tag["value"]);
    }
	...
	$description = trim(strip_tags($post->post_content));
...
function elegant_grunge_admin() {
	...
	<tr valign="top">
		<th scope="row"><?php _e('RSS subscription:') ?></th>
		<td>
			<input type="checkbox" name="show_rss" <?php echo (get_option('show_rss') ? "checked" : ""); ?> />
			 <?php _e('Display RSS subscription link') ?>
		</td>
	</tr>
	
	<tr valign="top">
		<th scope="row"><?php _e('Post info:') ?></th>
		<td>
	...
	<tr valign="top">
		<th scope="row"><?php _e('Copyright message:') ?></th>
		<td>
	...
	<tr valign="top">
		<th scope="row"><?php _e('Image frames:') ?></th>
		<td>
			<input type="checkbox" name="frame_all_images" <?php echo (get_option('frame_all_images') ? "checked" : ""); ?> />
			 <?php _e('Apply frame to all images') ?><br/>
			<small>If enabled, all images larger than <?php echo ELEGANT_GRUNGE_FRAME_MIN_WIDTH ?>x<?php echo ELEGANT_GRUNGE_FRAME_MIN_HEIGHT ?> 
				...
				This setting can be configured per-post and per-page, also.</small>
		</td>
	</tr>
	
	<tr valign="top">
		<th scope="row"><?php _e("Don't frame images with class:") ?></th>
		<td>
			...
			<br/><small>Separate multiple classes with commas ','</small>
		</td>
	</tr>
	
	<tr valign="top">
		<th scope="row"><?php _e('Extra header content:') ?></th>
		<td>
			<textarea style="width: 300px; height: 100px;" name="extra_header"><?php echo htmlspecialchars(get_option('extra_header')) ?></textarea><br/>
			<small>This can be used to add extra RSS feed links from your page, for example, such as a Twitter feed.</small>
		</td>
	</tr>
	
	<tr valign="top">
		<th scope="row"><?php _e('Photoblog thumbnails:') ?></th>
		<td>
			<p>
			<input type="checkbox" name="create_photoblog_thumbnails" <?php echo (get_option('create_photoblog_thumbnails') ? "checked" : ""); ?> />
			 <?php _e('Create scaled thumbnails') ?><br/>
			<small>If enabled, will generate thumbnail files. Otherwise, will use the original images, resulting in slower loading times.
			Note that the first photoblog page load after this is enabled will be slow, while images are being created, so you should
			<a href="/tag/photoblog" target="_blank">load this</a> yourself.</small>
			</p>
			<p>
			...
			<small>Leave blank for flexible size</small>
		</td>
	</tr>
	
	<tr valign="top">
		<th scope="row"><?php _e('Photoblog display:') ?></th>
		<td>
			<p>
			<?php _e('Number of thumbnails per page:') ?><br/>
			...
			<input type="checkbox" name="photoblog_lightbox" <?php echo (get_option('photoblog_lightbox') ? "checked" : ""); ?> />
			 <?php _e('Use lightbox mode') ?><br/>
			<small>Requires a lightbox plugin to be installed, such as <a href="http://www.stimuli.ca/lightbox/">Lightbox 2</a>.</small>
			</p>
			<p>
			<input type="checkbox" name="photoblog_frames" <?php echo (get_option('photoblog_frames') ? "checked" : ""); ?> />
			 <?php _e('Draw frames around photoblog items') ?><br/>
			</p>
		</td>
	</tr>
	...
	<input type="hidden" name="page_options" value="show_rss,show_author,copyright,frame_all_images,frame_class_skip,extra_header,create_photoblog_thumbnails,photoblog_thumb_count,photoblog_thumb_width,photoblog_thumb_height,photoblog_lightbox,photoblog_frames" />
...
function elegant_grunge_post_options() {
   ...
   <h3><?php _e('Elegant Grunge Theme Options') ?></h3>
   <div class="inside">
	<input value="eg_edit" type="hidden" name="eg_edit" />
	<table class="form-table">
	<tr>
		<th style="text-align:left;" colspan="2"><?php _e('Image frames:') ?></th>
		<td>
			...
			<small>If enabled, all images larger than <?php echo ELEGANT_GRUNGE_FRAME_MIN_WIDTH ?>x<?php echo ELEGANT_GRUNGE_FRAME_MIN_HEIGHT ?> 
				will have a frame with drop shadow applied. Otherwise, only images and other elements with a class
				of 'frame' will have this style applied.</small>
		</td>
	</tr>
	</table>
	</div>
...
function elegant_grunge_page_options() {
   ...
   <div class="inside">
	...
	<table class="form-table">
		...
		<tr>
			<th style="text-align:left;" colspan="2"><?php _e('Related tag(s):') ?></th>
			<td>
		...
		<tr>
			<th style="text-align:left;" colspan="2"><?php _e('Related posts title:') ?></th>
			<td>
		...
		<tr>
			<th style="text-align:left;" colspan="2"><?php _e('Extra sidebar content:') ?></th>
			<td>
...
function elegant_grunge_photoblog_setup() {
	...
	?>
	<p>
	<label for="photoblog_heading"><?php _e('Heading:') ?></label>
	...
	<p>
	<label for="photoblog_entries"><?php _e('Display:') ?></label>
	<input name="elegant_grunge_photoblog_entries" id="photoblog_entries" type="text" class="text" size="5" value="<?php echo get_option('elegant_grunge_photoblog_entries') ?>" /> entries<br/>
	</p>
	...
	<label for="photoblog_order_random"><?php _e('Display random entries') ?></label><br />
	...
	<label for="photoblog_order_recent"><?php _e('Display recent entries') ?></label><br />
	</p>
	
	<p>
	<label for="photoblog_tags"><?php _e('Tags to display:') ?></label>
	...
	<p>
	<label for="photoblog_width"><?php _e('Image width:') ?></label>
	...
	<p>
	<label for="photoblog_height"><?php _e('Image height:') ?></label>
...
add_filter( 'the_content', 'elegant_grunge_filter' );
