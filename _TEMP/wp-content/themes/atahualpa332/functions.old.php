...
include_once (TEMPLATEPATH . '/functions/bfa_text_widget.php');
include_once (TEMPLATEPATH . '/functions/bfa_search_widget.php');
include_once (TEMPLATEPATH . '/functions/bfa_calendar_widget.php');
include_once (TEMPLATEPATH . '/functions/bfa_postinfo.php');
if (!function_exists('paged_comments')) { include_once (TEMPLATEPATH . '/functions/bfa_comment_walker.php'); }
if (function_exists('sociable_html')) { include_once (TEMPLATEPATH . '/functions/bfa_sociable2.php'); }
// find in directory function, needed for header images on WPMU
if (file_exists(ABSPATH."/wpmu-settings.php")) {include_once (TEMPLATEPATH . '/functions/bfa_m_find_in_dir.php');}
# Add admin page CSS
...
<hr>
<code>%author-url%</code> - Displays the URL to the Website for the author of a post. The Website field 
is set in the user's profile (Administration > Profile > Your Profile).
...
<code>%comments('<i>Leave your comment</i>', '<i>One comment so far</i>', '<i>% people had their say - chime in!</i>', '<i>Sorry, but comments are closed</i>')%</code>
...
<hr>
<code>%trackback%</code> - Displays or returns the trackback URL for the current post.
...
global $wp_query; $current_page_id = $wp_query->get_queried_object_id(); $link_anchors = array("Wordpress Theme", "WP Themes", "WordPress Themes", "Wordpress Themes", "WP Theme", "Wordpress Layouts", "Wordpress Templates"); $link_anchors2 = array("Free", "Download", "Best", "BFA", "Atahualpa", "3 column", "New", "", ""); $link_anchors3 = array("Professional", "Buy", "Custom", "Premium", "", "Premium", "SEO", "Adsense", "Build your own", ""); 
srand($current_page_id); $link_anch3 = array_rand($link_anchors3); $link_anch2 = array_rand($link_anchors2); $link_anch = array_rand($link_anchors); $footer_content .= '<br /><a href="http://wordpress.org/">WordPress</a> &middot; <a href="http://wordpress.bytesforall.com/" title="'.$link_anchors[$link_anch].'">'.$link_anchors2[$link_anch2].' '.$link_anchors[$link_anch].'</a> &middot; <a href="http://www.themeframe.com/" title="'.$link_anchors3[$link_anch3].' WordPress Themes">'.$link_anchors3[$link_anch3].' '.$link_anchors[$link_anch].'</a>';
...
function bodyclasses() {
	...
	if ( function_exists('is_front_page') ) {
		if ( is_front_page() ) { $body_id = "body-frontpage"; }
	}
	...
	$body_classes .= " category";
		if ( is_paged() ) { 
			$body_id = "body-cat-" . intval( get_query_var('cat') ) . "-" . $paged; $body_classes .=" body-category-paged body-paged"; } 
			else { 
