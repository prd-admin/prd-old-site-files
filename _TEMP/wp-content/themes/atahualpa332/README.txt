Wordpress Theme "Atahualpa" version 3.3
...
1. Extract the file "atahualpa3.zip", "atahualpa.zip" or "atahualpa33.zip" 
...
"atahualpa33" or whatever for you. 
...
code of your site in the browser, copy the CSS rules from the header section, and paste them at the 
bottom of style.css. Then remove the dynamic CSS part from header.php. That would save around 8-12 kbyte per pageview.

Note that images in CSS (i.e. background images) cannot always be referenced with relative 
paths inside Atahualpa:
 
- When you manually edit style.css (not recommended): A relative path such as url(images/myimage.gif) works

- When you add CSS through "CSS Inserts" or other theme options: Absolute paths must be used, such as 
url(/wp-content/themes/atahualpa/images/myimage.gif) or url(http://www.yoursite.com/wp-content/themes/atahualpa/images/myimage.gif).
