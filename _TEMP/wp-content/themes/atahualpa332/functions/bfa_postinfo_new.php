...
// replace date format escape placeholders(#) with the actual escpae character (=backslashes)
// This function removes all backslashes from post info strings to avoid issues with hosts that have magic_quotes_gpc ON 
// but we want to keep the backslashes inside date items, because they are needed to escape literal strings inside dates
$postinfo_string = str_replace("#", "\\", $postinfo_string);

// Author public name
if (strpos($postinfo_string,'%author%')!==FALSE) {
ob_start(); the_author(); $author = ob_get_contents(); ob_end_clean();
$postinfo = str_replace("%author%", $author, $postinfo);
}

// Author about yourself
if (strpos($postinfo_string,'%author-description%')!==FALSE) {
ob_start(); the_author_description(); $author_description = ob_get_contents(); ob_end_clean();
$postinfo = str_replace("%author-description%", $author_description, $postinfo);
}

// Author login name
if (strpos($postinfo_string,'%author-login%')!==FALSE) {
ob_start(); the_author_login(); $author_login = ob_get_contents(); ob_end_clean();
$postinfo = str_replace("%author-login%", $author_login, $postinfo);
}

// Author first name
if (strpos($postinfo_string,'%author-firstname%')!==FALSE) {
ob_start(); the_author_firstname(); $author_firstname = ob_get_contents(); ob_end_clean();
$postinfo = str_replace("%author-firstname%", $author_firstname, $postinfo);
}

// Author last name
if (strpos($postinfo_string,'%author-lastname%')!==FALSE) {
ob_start(); the_author_lastname(); $author_lastname = ob_get_contents(); ob_end_clean();
$postinfo = str_replace("%author-lastname%", $author_lastname, $postinfo);
}

// Author nickname
if (strpos($postinfo_string,'%author-nickname%')!==FALSE) {
ob_start(); the_author_nickname(); $author_nickname = ob_get_contents(); ob_end_clean();
$postinfo = str_replace("%author-nickname%", $author_nickname, $postinfo);
}

// Author ID
if (strpos($postinfo_string,'%author-id%')!==FALSE) {
ob_start(); the_author_ID(); $author_ID = ob_get_contents(); ob_end_clean();
...
ob_start(); the_author_email(); $author_email_clear = ob_get_contents(); ob_end_clean();
...
ob_start(); the_author_url(); $author_url = ob_get_contents(); ob_end_clean();
...
ob_start(); the_author_posts_link(); $author_posts_link = ob_get_contents(); ob_end_clean();
$postinfo = str_replace("%author-posts-link%", $author_posts_link, $postinfo);
}

// Author post count
if (strpos($postinfo_string,'%author-posts%')!==FALSE) {
ob_start(); the_author_posts(); $author_posts = ob_get_contents(); ob_end_clean();
$postinfo = str_replace("%author-posts%", $author_posts, $postinfo);
}

// Author AOL Instant Messenger screenname
if (strpos($postinfo_string,'%author-aim%')!==FALSE) {
ob_start(); the_author_aim(); $author_aim = ob_get_contents(); ob_end_clean();
$postinfo = str_replace("%author-aim%", $author_aim, $postinfo);
}

// Author Yahoo IM ID
if (strpos($postinfo_string,'%author-yim%')!==FALSE) {
ob_start(); the_author_yim(); $author_yim = ob_get_contents(); ob_end_clean();
...
ob_start(); the_time($date_matches[2]); $date = ob_get_contents(); ob_end_clean();
...
ob_start(); the_modified_time($date_matches[2]); $date_modified = ob_get_contents(); ob_end_clean();
...
ob_start(); get_the_tag_list($tag_link_matches[2], $tag_link_matches[4], $tag_link_matches[6]); 
$tags_linked = ob_get_contents(); ob_end_clean();
$postinfo = preg_replace("/(.*)%tags-linked\((.*?)\)%(.*)/i", "\${1}" .$tags_linked. "\${3}", $postinfo);
}

// Tags, linked. If post has no tags, categories are displayed instead
...
ob_start(); the_tags($tag_link_matches[2], $tag_link_matches[4], $tag_link_matches[6]); 
$tags_cats_linked = ob_get_contents(); ob_end_clean();
...
ob_start(); the_category($category_matches[2]); $categories_linked = ob_get_contents(); ob_end_clean();
...
if ( !comments_open() AND $comment_matches[8] == "dontshow" ) { $comment_link = ''; }
else { ob_start(); comments_popup_link($comment_matches[2], $comment_matches[4], $comment_matches[6], 'comments-link', $comment_matches[8]);
$comment_link = ob_get_contents(); ob_end_clean(); }
...
if ( function_exists('post_comments_feed_link') ) { post_comments_feed_link($comments_rss_matches[2]); } 
else { comments_rss_link($comments_rss_matches[2]); }
$comments_rss_link = ob_get_contents(); ob_end_clean();
// make link nofollow if set in theme options
if ( $cur_opt['bfa_ata_nofollow'] == "Yes" ) {
...
ob_start(); trackback_rdf(); 
$trackback_rdf = "<!-- " . ob_get_contents() . " -->"; ob_end_clean();
$postinfo = str_replace("%trackback-rdf%", $trackback_rdf, $postinfo);
}

// Permalink
if (strpos($postinfo_string,'%permalink%')!==FALSE) {
ob_start(); the_permalink(); $permalink = ob_get_contents(); ob_end_clean();
...
ob_start(); the_title(); $post_title = ob_get_contents(); ob_end_clean();
...
ob_start(); edit_post_link($edit_matches[4], $edit_matches[2], $edit_matches[6]); $edit_link = ob_get_contents(); ob_end_clean();
...
ob_start();
if (function_exists('sharethis_button')) { sharethis_button(); $share_this = ob_get_contents(); } 
else { $share_this = ""; }
ob_end_clean();
...
// The meta = ALL custom fields:values, formatted by Wordpress as unordered list <ul><li>..</li><li>..</li></ul>
if (strpos($postinfo_string,'%meta%')!==FALSE) {
ob_start(); the_meta(); $the_meta = ob_get_contents(); ob_end_clean();
