...
if (strpos($header_items,'%logo')!==false) {
	...
	$bfa_ata['show_email_icon'] == "Yes" OR $bfa_ata['show_comments_icon'] == "Yes")) { 
		...
		if ($bfa_ata['show_comments_icon'] == "Yes" ) { 
			...
			if ($bfa_ata['nofollow'] == "Yes") { $logo_area .= 'rel="nofollow" '; } 
			$logo_area .= 'href="' . get_bloginfo('comments_rss2_url') . '" title="' . 
...
if (strpos($header_items,'%image')!==false) {
	...
	$header_image = '<div class="header-image-container" style="background: url(' . $selected_header_image . ') ' . 
		$bfa_ata['headerimage_alignment'] . ' no-repeat;">';
	$header_image .= ($bfa_ata['header_image_clickable'] == "Yes" ? '<div class="clickable">
		...
		if ( $bfa_ata['overlay_blog_title'] == "Yes" OR $bfa_ata['overlay_blog_tagline'] == "Yes" ) {
			...
			( $bfa_ata['overlay_blog_title'] == "Yes" ? '<h1 class="blogtitle"><a href="' . get_option('home') . '/">' . 
			get_bloginfo('name') . '</a></h1>' : '' ) . ( $bfa_ata['overlay_blog_tagline'] == "Yes" ? '<p class="tagline">' . 
